...
-(void)setSourcePosition:(Point2D*)p
{
	[sourcePosition setX:p.x y:p.y];
}
...
- (int)update:(float)deltaT
{
	...
	while(particleIndex < particleCount) 
	{
		...
		if(currentParticle->timeToLive > 0) 
		{
			...
			currentParticle->color.red += (currentParticle->deltaColor.red * deltaT);
			currentParticle->color.green += (currentParticle->deltaColor.green * deltaT);
			currentParticle->color.blue += (currentParticle->deltaColor.blue * deltaT);
			currentParticle->color.alpha += (currentParticle->deltaColor.alpha * deltaT);
...
- (void)dealloc 
{
	...
	[sourcePosition dealloc];
	[sourcePositionVariance dealloc];
	if(gravity)
		[gravity dealloc];
	if(texture)
		[texture release];
